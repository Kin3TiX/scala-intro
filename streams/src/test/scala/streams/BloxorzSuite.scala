...
class BloxorzSuite extends FunSuite {
  ...
  test("neighborsWithHistory level 1") {
    new Level1 {
      val expectedNeighbors = Set(
        (Block(Pos(1,2),Pos(1,3)), List[Move](Right,Left,Up)),
        (Block(Pos(2,1),Pos(3,1)), List[Move](Down,Left,Up))
      )
      val neighbors = neighborsWithHistory(Block(Pos(1,1),Pos(1,1)), List(Left,Up)).toList
      assert(neighbors.forall(neighbor => expectedNeighbors(neighbor)))
    }
  }
  ...
  test("newNeighborsOnly level 1") {
    new Level1 {
      ...
      assert(newNeighbors == List((Block(Pos(2,1),Pos(3,1)), List[Move](Down,Left,Up))))
    }
  }
